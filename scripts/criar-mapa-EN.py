...
df_empresas = pd.read_excel(f'{dir}/dados/empresas_en.xlsx', sheet_name=0)
...
for index, row in df_empresas.iterrows():
    content_html = template_html.format(
        ...
        segmento = row['SECTOR_OF_ACTIVITY'],
        historia = row['BRIEF_HISTORY'],
        img = row['IMAGE'],
        funcionarios = row['NUM_EMPLOYEES'],
        tipoempresa = row['COMPANY_TYPE'],
        enconomia = row['LOCAL_ECONOMY_CONTRIBUTION'],
        premios = row['CERTIFICATIONS_AND_AWARDS'],
        abrangencia = ['PRODUCTION_COVERAGE']
    )
